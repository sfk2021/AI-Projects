...
# Frontend (HTML + CSS + JS) — all inline using render_template_string
HTML_PAGE = """
<!doctype html>
<html lang="en">
<head>
...
async function sendMessage(){
  ...
  try{
    ...
    if(res.ok && data.reply){
      ...
      appendMessage('Error: ' + (data.error || data.reply || res.statusText), 'bot');
      setStatus('Error');
...
def ping():
    # quick connectivity test hitting the base endpoint (no key leak)
    try:
        # This uses the client to get a simple response; we don't pass confidential data
        # Use a minimal request with input string
        resp = client.responses.create(model=DEPLOYMENT, input="ping")
        # return status and small snippet
        snippet = getattr(resp, "output_text", None) or str(getattr(resp, "output", resp))[:200]
